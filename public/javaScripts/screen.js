...
const handleScreenDeletion = async (screenid) => {
  try {
    ...
    Swal.fire("Deleted!", "Your screen has been deleted.", "success").then(
      () => {
...
async function restoreScreen(screenid) {
  try {
    ...
    if (data.success) {
      Swal.fire("Restored!", "Your screen has been restored.", "success").then(
        () => {
...
function filterScreens() {
  ...
  rows.forEach(function (row) {
    var screenid = row
      .querySelector('td[id^="screen.screenid"]')
      .textContent.toUpperCase();
    var screenName = row
      .querySelector('td[id^="screen.screenname"]')
      .textContent.toUpperCase();
    var screenType = row
      .querySelector('td[id^="screen.ifsecondscreenispresentondevice"]')
      .textContent.toUpperCase();
    var playlistName = row
      .querySelector('td[id^="screen.playlistname"] p')
      .textContent.toUpperCase();
    var filled_slots = row
      .querySelector('td[id^="screen.filled_slots"]')
      .textContent.toUpperCase();
    var status = row
      .querySelector('td[id^="screen.deleted"] p')
      .textContent.toUpperCase();
    var resolution = row
      .querySelector('td[id^="screen.resolution"]')
      .textContent.toUpperCase();
    // var tags = row.querySelector('td[id^="screen.tags"]').textContent.toUpperCase();
    var location = row
      .querySelector('td[id^="screen.location"]')
      .textContent.toUpperCase();
    // var city = row.querySelector('td[id^="screen.city"]').textContent.toUpperCase();
    var pincode = row
      .querySelector('td[id^="screen.pincode"]')
      .textContent.toUpperCase();

    if (
      screenid.indexOf(filter) > -1 ||
...
function deletePlaylist(screenid) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire("Cancelled", "Your playlist is safe :)", "info");
    }
  });
}
